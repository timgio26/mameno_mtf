...
import { useAddData } from "../utilities/myQuery";
...
export function NewForm({setShowModal}:NewFormProp) {
  ...
  const {mutate,isPending} = useAddData(kategori)
  ...
  return (
    <div className="space-y-3 max-h-[80vh] overflow-y-auto">
      ...
      {/* Divisi List */}
      {kategori=="bersama"&&
      <div className={`flex flex-col ${divisiList.length>0?"gap-2":"gap-1"}`}>
        <label
          htmlFor="divisi"
          className="text-sm font-semibold text-slate-700"
        >
        ...
        <div className="space-y-1">
          {divisiList.map((each, index) => (
            ...
            >
              <span className="text-sm font-medium text-slate-700">{each}</span>
              <button
                type="button"
